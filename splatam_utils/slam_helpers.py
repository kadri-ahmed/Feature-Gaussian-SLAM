...
def transformed_params2rendervar(params, transformed_gaussians, color_offset, quantized_features=None, device="cuda", requires_grad=True):
    # Check if Gaussians are Isotropic
    if params['log_scales'].shape[1] == 1:
        log_scales = torch.tile(params['log_scales'], (1, 3))
    else:
        log_scales = params['log_scales']
    # Initialize Render Variables
    # color_offset determines if we render colours or features
    # offset has to be either 0, 3, or 6
    if color_offset == 0 or (color_offset != 3 and color_offset != 6):
        precomp_colors = params['rgb_colors']
    else:
        if color_offset == 3:
            if quantized_features == None:
                precomp_colors = params['semantic_features'][:,0:3]
            else:
                precomp_colors = quantized_features
        else:
            # in that case color_offset has to be 6
            if quantized_features == None:
                precomp_colors = params['semantic_features'][:,3:6]
            else:
                precomp_colors = quantized_features
    rendervar = {
        'means3D': transformed_gaussians['means3D'],
        'colors_precomp': precomp_colors,#params['rgb_colors'],
        'rotations': F.normalize(transformed_gaussians['unnorm_rotations']),
        'opacities': torch.sigmoid(params['logit_opacities']),
        'scales': torch.exp(log_scales),
        'means2D': torch.zeros_like(params['means3D'], requires_grad=requires_grad, device=device) + 0
    }
    return rendervar
...
def transform_to_frame(params, time_idx, gaussians_grad, camera_grad):
    ...
    rotation_matrix = build_rotation(cam_rot)
    ...
    transformed_pts = ((rotation_matrix[0] @ pts.T).T + cam_tran)#rotation_matrix @ pts.T + cam_tran.unsqueeze(-1)
