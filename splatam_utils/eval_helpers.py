...
def report_loss(losses, wandb_run, wandb_step, tracking=False, mapping=False, mapping_og=False):
    # Update loss dict
    if not mapping_og:
        ...
        if 'separation' in losses:
            loss_dict['Separation Loss'] = losses['separation']
        if 'cohesion' in losses:
            loss_dict['Cohesion Loss'] = losses['cohesion']
        if 'clustering_coarse' in losses:
            loss_dict['Coarse Clustering Loss'] = losses['clustering_coarse']
        if 'clustering_fine' in losses:
            loss_dict['Fine Clustering Loss'] = losses['clustering_fine']
...
def report_progress(params, data, i, progress_bar, iter_time_idx, sil_thres, every_i=1, qual_every_i=1, 
    ...
    if i % every_i == 0 or i == 1:
        ...
        rendervar = transformed_params2rendervar(params, transformed_gaussians, 0)
...
def eval(dataset, final_params, num_frames, eval_dir, sil_thres, 
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        lpips_score = None#loss_fn_alex(torch.clamp(weighted_im.unsqueeze(0), 0.0, 1.0),
