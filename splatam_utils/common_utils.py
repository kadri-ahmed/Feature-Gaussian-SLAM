...
def save_params_ckpt(output_params, assignments, cluster_masks  , output_dir, time_idx):
    # Convert to CPU Numpy Arrays
    to_save = params2cpu(output_params)
    to_save['cluster_assignments'] = assignments.detach().cpu().contiguous().numpy()
    to_save['cluster_masks'] = cluster_masks
    # Save the Parameters containing the Gaussian Trajectories
    os.makedirs(output_dir, exist_ok=True)
    print(f"Saving parameters to: {output_dir}")
    save_path = os.path.join(output_dir, "params.npz")
